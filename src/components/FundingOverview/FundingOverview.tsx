...
import { getResourceLabel } from "../../utils/strings";
  ...
  <div>
    <HeaderContainer>
      <SectionHeader
        ...
        subtitle={
          <>
            Totals funds made available{" "}
            {snapshotOwner ? `to the ${snapshotOwner}` : ""} over its entire
            lifetime
            {sinceDate ? (
              <>
                , since{" "}
                <b>{DateTime.fromJSDate(sinceDate).toFormat("LLLL yyyy")}</b>
              </>
            ) : (
              ""
            )}
        ...
        tooltip={`Monitor funds made available to ${getResourceLabel(
          resourceType
        )}s, track spending, returns, \
    ...
    <CardsContainer>
      <SimpleStatCard
        date={startDate}
        value={
          typeof balance?.initialBalance === "number"
            ? Math.abs(balance?.initialBalance ?? 0)
            : undefined
        }
      ...
      <FundChangeCard
        netChange={
          typeof balance?.inflow === "number" &&
          typeof balance?.outflow === "number"
      ...
      <SimpleStatCard
        date={endDate}
        value={
          balance?.newBalance ? balance.newBalance * -1 : balance?.newBalance
        }
