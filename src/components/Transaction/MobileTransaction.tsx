  ...
  return (
    ...
    >
      <TransactionSummary expanded={expanded}>
        <ArrowContainer>
          {isIncomingTransaction ? <GreenArrowDown /> : <RedArrowUp />}
        </ArrowContainer>
        <Content>
          <Data>
          ...
          <Value
            isLight={isLight}
            isGreen={amount > 0 && !!highlightPositiveAmounts}
          >
            <Sign>{amount < 0 ? '-' : '+'}</Sign>
...
const TransactionSummary = styled((props: AccordionSummaryProps) => (
...
const Value = styled.div<WithIsLight & { isGreen: boolean }>(
  ({ isLight, isGreen }) => ({
    display: 'flex',
    alignItems: 'baseline',
