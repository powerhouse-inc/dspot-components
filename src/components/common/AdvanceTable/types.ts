...
import { breakpoints } from '../../../styles/theme/light'
...
export type CellPadding =
  | string
  | number
  | Record<keyof typeof breakpoints, number | string>
...
export interface TableProps {
  ...
  headerRender?: React.FC<
    { header: TableProps['header'] } & React.PropsWithChildren
  >
...
export const isBorder = (border: BorderConfig | Border): border is Border => {
  ...
  return (
    ...
    (keys.includes('width') || keys.includes('style') || keys.includes('color'))
  )
}
