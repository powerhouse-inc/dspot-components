...
import { Snapshots, Token } from './types'
...
const useAccountsSnapshot = (snapshot: Snapshots) => {
  ...
  const enableCurrencyPicker = false
  ...
  const rootAccount = useMemo(
    () =>
      snapshot.snapshotAccount.find(
        (account) =>
          account.groupAccountId === null && account.upstreamAccountId === null
      ),
  ...
  const mainAccount = useMemo(
    () =>
      snapshot.snapshotAccount.find(
        (account) =>
          account.groupAccountId === rootAccount?.id &&
  ...
  const mainBalance = useMemo(
    () =>
      mainAccount?.snapshotAccountBalance?.find(
  ...
  const cuReservesBalance = useMemo(
    () =>
      (cuReservesBalances?.length ?? 0) > 1
        ? cuReservesBalances?.find(
            (account) => account.includesOffChain === includeOffChain
          )
  ...
  const actualsComparison = useMemo(
    () =>
      snapshot.actualsComparison
        ...
        .sort(
          (a, b) =>
            DateTime.fromFormat(a.month, 'yyyy/MM').diff(
              DateTime.fromFormat(b.month, 'yyyy/MM')
            ).months
        ),
